...
> 连游戏自带的**防作弊校验**一起修好。
>
> 作者 **[@huxc573](https://github.com/huxc573)** · 开源协议 **MIT** · 当前版本 **v0.4**
...
| 写回前自检 + 自动 `*.bak.<时间戳>` 备份 | ✅ |
| **打包成 exe**（`tools\build.py`，自带 32 位宿主） | ✅ |
| 物品批量增删 / 图形化编辑技能装备 | ⏳ 见 `docs\待解决问题.md` |
...
背包 / 物品      4 页×20 格；改数量/清空/添加（自动同步物品计数校验）
...
打包产物在 `dist\`：`画迹2存档工具v0.4.exe` + `XJCodec32.exe`（**必须挨着 exe**，
