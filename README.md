...
> 用前请先备份存档。
>
> 作者 **[@huxc573](https://github.com/huxc573)** · 开源协议 **MIT** · 当前版本 **v0.5.0**
...
打包产物在 `dist\`：`画迹2存档工具v0.5.0.exe` + `XJCodec32.exe`（**必须挨着 exe**，
...
⚠ 两种“校验”别搞混：「校验码修复（Lock 校验和）」修的是存银/步数旁边那套
`Lock`（`v*91+45+seed/800`）；「作弊标记 `@cheated`」要靠「清除作弊标记」/
